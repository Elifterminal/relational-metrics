...
| `EXP-051` | Does relational structure have holography's SECOND property — does every fragment carry the whole? | **The analogy decomposes, and half of it isn't about relations.** First pass looked like strong confirmation (40% of relational terms → 0.924 discrimination; *all* property terms → 0.642 ceiling). **The matched-term-count control killed it**: at equal counts property terms are just as informative, slightly more (0.698 vs 0.651). The advantage was pool size, not distributedness. Sharper test: one property coefficient carries **3.048 bits**, one relational coefficient **3.021**, one raw value **0.999** — **distributedness belongs to the transform domain, not to relational content.** The thesis is half right and half misattributed |
| `EXP-052` | The type-substitution search can invert polarity, matching a reinforcing loop to a balancing one. Does constraining it to preserve declared polarity change any published result? | **A real hole in the measure, and a control that had been dead for eighteen experiments.** The unconstrained search distinguished a structure from its sign-flipped twin **0 times out of 40 at every size from 2 to 8 relations** — a global sign flip is a bijection on types, so the swap always matches perfectly. No size or symmetry condition exists; the two mitigating claims in my own Q-41 note were both false. The freedom is exercised on **30–47% of real corpus pairs**. A **polarity-preserving** constraint (weaker than name-preserving: permits POS→GROWS, refuses POS→NEG) removes it, preserves cross-vocabulary correspondence exactly, and changes **no published verdict** (analogue > false friend 10/10 → 10/10) — but moves rankings on 6 motifs of 10 and scores on 9 of 10, so it is a question about **confidence, not conclusions**. Separately, the plan's code-robustness rule caught that **`flat32` has charged 32 bits to match a relation against a 6-bit saving since EXP-034 added the weight channel**, so its best mapping matches zero edges on every pair. Third control to be valid when written and silently killed by a later change → `P-26` |
| `EXP-053` | If the polarity-preserving constraint were adopted as the measure's default, which published results would move? | **42 verdict changes across 26 runners — and nearly every one flatters the project.** `EXP-039` goes from `FRAGILE, NOT ESTABLISHED` (leave-one-out 3/9) to `SUPPORTED` (9/9); `EXP-038` from 5/10 to 10/10; **`EXP-028`'s retracted re-encoding claim flips back to True on 4 rows**. The predeclared band at 4+ changes says **DO NOT ADOPT** — a constraint moving this much of the record is a different measure, not a correction, and re-enters at rung 1. There is a coherent mechanism (distractors *need* sign inversion to match; genuine analogues do not), and a mechanism that predicts the pattern is the minimum bar for investigating it, not evidence it is real. **Separately the audit found four committed results that no longer reproduce** — `exp009.json` stores 1.727 where its code produces 1.4059, stranded by `EXP-024`'s encoding correction. `check_manifest` verified a result file *exists*, never that it was *current* — the manifest machinery had the defect it was built to catch (`P-26`, fourth instance). **Full sweep: 47 checked, 42 reproduce, 5 stale** (`exp014` was invisible to the partial audit — its runner never calls the measure). **No published claim changes** — `EXP-030`'s headline stays `NO SIGNAL`, so this is a hygiene failure, not a retraction. The five were regenerated with their published contents kept verbatim in `results/superseded/2026-07-27/`. Then the repair had the same bug twice: the first tripwire flagged all 50 files on any measure touch, and the sweep let a subset run overwrite the full report — **six instances in one session, two inside the machinery built to catch the other four** |
| `EXP-054` | Q-44 — does the polarity-constrained measure earn adoption from rung 1, with nothing inherited? | **No. Adoption refused by a control built to kill it.** The polarity filter permits 50.17% of type substitutions, so 200 *arbitrary* filters permitting the same fraction were run against `EXP-039`'s leave-one-out. Unconstrained 3/9, polarity-constrained 9/9, **random restrictions median 0/9 — but 33 of 200 matched or beat it**. That puts polarity at the **83.5th percentile against a predeclared threshold of the 95th**. Both arms agree. So most ways of crippling the search hurt and this one genuinely helps, but **one arbitrary restriction in six helps as much** — too many to credit polarity rather than luck. Parts A, B and D all passed and none of them mattered; C had veto power, declared before running. **`EXP-053`'s 42 changes are an artifact of search restriction, not a discovery about polarity.** The degeneracy stays real, the default stays unconstrained, every published number stands. Separately, an impostor comparing only the *fraction of amplifying relations* — blind to structure — scored **9/10** on the retrieval claim → `Q-46`: the corpus may be separable by polarity profile alone |
| `EXP-055` | Q-42 — holography's fragment is a SPATIAL region. Under a structural fragment (a subset of participants), is a relational record distributed where a property record is not? | **No — and this closes the founding analogy at half its original size.** All 65,536 Boolean functions enumerated exactly; arms **matched from the start** per `Q-42`'s predeclared caution. A structural fragment **never uniquely determines anything it cannot see** — 0.0000 at both fragment sizes for *both* relational and property records. Paired differences on the continuous statistic: d = 0.012 and d = −0.025, **in opposite directions**, both CIs spanning zero. **The control is the interesting part**: measurements drawn from *anywhere*, unconfined to a region, determine far more (0.170 vs 0.000). So being confined to a region is what destroys recoverability — equally for both kinds of record — which is precisely backwards from a hologram, where any region carries the whole. Two independent routes now reach the same negative (`EXP-051` coefficient fragments, `EXP-055` spatial fragments). **Interference-records-what-intensity-discards holds (`C-01`); every-fragment-carries-the-whole does not.** |
...
python3 run_exp051.py    # the holographic analogy decomposes -- one half isn't ours
python3 run_exp052.py    # the measure cannot tell reinforcement from balance -- and a stress code that died in EXP-034
python3 run_exp053.py    # 42 verdicts move and all of them favour us -- so do not adopt
python3 run_exp054.py    # the improvement was restriction, not polarity -- refused
python3 run_exp055.py    # the second holographic property fails under the RIGHT notion of fragment too
